package au.com.escybersourceconnector.operationhandlers.refund;
...
import au.com.escybersourceconnector.validators.RefundValidators;
...
public class RefundCaptureHandler implements OperationHandler {
    ...
    private Mono<ServerResponse> makeWebClientCall(ExchangeContext exchangeContext) {
        return cybersourceWebClient
                .post()
                .uri(cyberSourceEndpoints.get(Operation.CAPTURE),
                        exchangeContext.getServerRequest().pathVariable("id"))
    ...
    private Mono<ServerResponse> processSuccessResponse(ClientResponse clientResponse, ExchangeContext exchangeContext) {
        return clientResponse.toEntity(CyberSourceResponse.class)
                .doOnNext(response -> log.info("timeout refund capture successful: {}", response.getBody()))
                .flatMap(response -> {
    ...
    private Mono<ServerResponse> processErrorResponse(ClientResponse clientResponse, ExchangeContext exchangeContext) {
        return clientResponse.toEntity(CyberSourceResponse.class)
                ...
                .onErrorResume(WebClientResponseException.class, error -> {
                    log.error("Error processing refund capture request: {}", error.getMessage(), error);
